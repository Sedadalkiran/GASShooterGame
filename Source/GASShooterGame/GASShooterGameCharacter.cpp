...
AGASShooterGameCharacter::AGASShooterGameCharacter()
{
...
void AGASShooterGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("Move Forward / Backward", this, &AGASShooterGameCharacter::MoveForward);
	PlayerInputComponent->BindAxis("Move Right / Left", this, &AGASShooterGameCharacter::MoveRight);

	// We have 2 versions of the rotation bindings to handle different kinds of devices differently
	// "turn" handles devices that provide an absolute delta, such as a mouse.
	// "turnrate" is for devices that we choose to treat as a rate of change, such as an analog joystick
	PlayerInputComponent->BindAxis("Turn Right / Left Mouse", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("Turn Right / Left Gamepad", this, &AGASShooterGameCharacter::TurnAtRate);
	PlayerInputComponent->BindAxis("Look Up / Down Mouse", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("Look Up / Down Gamepad", this, &AGASShooterGameCharacter::LookUpAtRate);
...
void AGASShooterGameCharacter::AddCharacterAbilities()
{
	if (GetLocalRole() == ROLE_Authority && IsValid(AbilitySystemComponent) && !AbilitySystemComponent->bCharacterAbilitiesGiven)
	{
		for (TSubclassOf<UGSGGameplayAbility>& StartupAbility : DefaultAbilities)
		{
...
void AGASShooterGameCharacter::BeginPlay()
{
	Super::BeginPlay();
...
void AGASShooterGameCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
	StopJumping();
}

void AGASShooterGameCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * TurnRateGamepad * GetWorld()->GetDeltaSeconds());
}

void AGASShooterGameCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
...
void AGASShooterGameCharacter::MoveRight(float Value)
{
	if ( (Controller != nullptr) && (Value != 0.0f) )
	{
